    ...
    <div className="cards">
      ...
      <div className="card">
        ...
        <div className="card-sub">
          Avg margin{" "}
          <span className="mini">{Number(avgMargin).toFixed(1)}%</span>
        </div>
      </div>
    </div>
  );
    ...
    <div className="grid3">
      ...
      {items.length === 0 && (
        <div className="empty-state">
          No source status data yet. Backend route{" "}
          <code>/api/status/sources</code> may not be implemented (or returned
          empty).
        </div>
      )}
    </div>
        ...
        <tbody>
          {runs.map((r) => (
            <tr key={r.id || `${r.keyword}-${r.startedAt}`}>
              <td>
                {new Date(r.startedAt || r.createdAt || Date.now()).toLocaleString()}
              </td>
              <td className="mono">{r.keyword}</td>
              <td>
                <Pill tone="neutral">{r.source || "unknown"}</Pill>
              </td>
...
function ImportForm({ onRunCompleted, onLog }) {
  ...
  const submit = async (e) => {
    ...
    if (!res) {
      ...
      onLog?.(
        `✅ Import finished: created ${res.createdCount ?? 0} products (runId=${
          res.runId ?? "n/a"
        })`
      );
      onRunCompleted?.();
    ...
    <div className="card">
      <div className="row between">
        <div>
          ...
          <div className="mini">
            Streaming from <code>/api/logs/live</code> (SSE)
          </div>
        </div>
...
export default function DashboardApp() {
  ...
  // ✅ NEW: load fulfillment data
  const refreshFulfillment = async () => {
    setFulfillmentLoading(true);
    const res = await apiGet("/api/fulfillment");
    setFulfillmentRows(res?.rows || res || []);
    setFulfillmentLoading(false);
  };

  // ✅ NEW: retry handler for a single fulfillment row
  const retryFulfillment = async (id) => {
    const ok = await apiPost(`/api/fulfillment/${id}/retry`, {});
    if (ok) pushLog(`♻️ Retry requested for fulfillment id=${id}`);
  ...
  const refreshAll = async () => {
    ...
    // ✅ also refresh fulfillment in the background
    refreshFulfillment();
  };
  ...
  // ✅ Live logs using SSE (NOT WebSocket)
  useEffect(() => {
  ...
  // ✅ when switching to fulfillment tab, load it if empty
  useEffect(() => {
    if (tab === "fulfillment" && fulfillmentRows.length === 0) {
      refreshFulfillment();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tab]);
        ...
        {tab === "fulfillment" && (
          <div className="section">
            <div className="section-header">
              <div>
                <h3>Fulfillment</h3>
                <div className="mini">Shopify → CJ → Delivered (auto-updating)</div>
              </div>
              ...
              <div className="row gap">
                ...
                >
                  {fulfillmentLoading ? "Loading…" : "Refresh"}
                </button>
              </div>
            </div>

            {/* ✅ THIS is where FulfillmentTable goes */}
            <FulfillmentTable
    ...
    <div className="tokenBox">
      ...
      <button
        ...
        onClick={() => {
          ...
          onSaved?.();
          // reload so SSE reconnects with token
